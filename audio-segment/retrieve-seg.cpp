...
__PROCESS_BOTH__ double value_from_segment(
  ...
  if (segment->values.segment.start_time < offset) {
    gka_local_address_t nextlp =
        gka_entry_next(blk, gka_to_local(blk, segment), GKA_SEGMENT_VALUE);
    if (nextlp) {
      next = gka_pointer(blk, nextlp);
    }
  }
...
__PROCESS_HOST__ int gka_count_sounds_in_block(struct gka_entry *blk) {
